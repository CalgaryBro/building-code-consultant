...
function CapacityGauge({ value, max = 100, label, status }: {
  ...
  const colors = {
    pass: { ring: '#10b981', glow: 'rgba(16, 185, 129, 0.3)' },
    warning: { ring: '#f59e0b', glow: 'rgba(245, 158, 11, 0.3)' },
    fail: { ring: '#ef4444', glow: 'rgba(239, 68, 68, 0.3)' },
  };
    ...
    <div className="relative flex flex-col items-center">
      <svg width="120" height="70" viewBox="0 0 120 70" className="overflow-visible">
        ...
        <path
          ...
          style={{
            filter: `drop-shadow(0 0 4px ${colors[status].glow})`,
          }}
        />
        {/* Needle */}
      ...
      <div className="text-center -mt-1">
        <div className="font-mono text-xl font-bold text-gray-900">{value.toFixed(1)}%</div>
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b border-gray-200">
            ...
            {data.return_periods.map((rp) => (
              <th key={rp} className="px-3 py-2 text-right text-amber-600 font-semibold">
                {rp}yr
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {data.durations_min.slice(0, 8).map((dur) => (
            <tr key={dur} className="border-b border-gray-100 hover:bg-amber-50/50">
              <td className="px-3 py-2 text-gray-700 font-medium">{dur} min</td>
              {data.return_periods.map((rp) => (
                <td key={rp} className="px-3 py-2 text-right text-gray-600">
...
function ResultsCard({ title, children, status }: {
  ...
  const borderColor = status === 'pass' ? 'border-emerald-200' : status === 'fail' ? 'border-red-200' : 'border-gray-200';
  const bgColor = status === 'pass' ? 'bg-emerald-50/50' : status === 'fail' ? 'bg-red-50/50' : 'bg-white';
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <select
        ...
        className="
          text-sm px-3 py-1.5 bg-white border border-gray-300
          rounded-lg text-gray-700 font-medium
          focus:outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20
          cursor-pointer shadow-sm hover:border-gray-400
        "
      >
...
// Default states
const DEFAULT_STORMWATER: StormwaterState = {
  pipeId: 'ST-1',
  catchments: [{
    catchment_id: 'C1',
    area_ha: 2.5,
...
const DEFAULT_SANITARY: SanitaryState = {
  pipeId: 'SAN-1',
  loads: [{
    load_id: 'L1',
    land_use: 'single_family',
...
const DEFAULT_WATER: WaterState = {
  serviceId: 'WM-1',
  loads: [{
    load_id: 'W1',
    land_use: 'single_family',
  ...
  lengthM: 150,
  availablePressure: 400,
  elevationChange: 0,
  material: 'ductile_iron_new',
  result: null,
};

// ============================================================================
// MAIN TABS (Now receive props from parent)
// ============================================================================
...
function StormwaterTab({
  ...
  state: StormwaterState;
  setState: React.Dispatch<React.SetStateAction<StormwaterState>>;
  presets: PresetScenario[] | undefined;
  ...
  const handleExportReport = useCallback(async () => {
    ...
    try {
      const html = await reportsApi.generateStormwaterReportHtml({
        ...
        return_period_years: state.returnPeriod,
      });
  ...
  }, [state.result, projectName, state.catchments, state.returnPeriod]);

  const handleCalculate = () => {
    calculate({
      pipe_id: state.pipeId,
      catchments: state.catchments,
      slope_percent: state.slopePct ? parseFloat(state.slopePct) : undefined,
      return_period_years: state.returnPeriod,
      material: state.material,
      min_diameter_mm: state.minDiameter,
    });
  };

  const updateCatchment = (index: number, field: keyof CatchmentInput, value: string | number) => {
    setState((prev) => {
      const updated = [...prev.catchments];
      updated[index] = { ...updated[index], [field]: value };
      return { ...prev, catchments: updated };
    });
  };
  ...
  const addCatchment = () => {
    setState((prev) => ({
      ...prev,
      catchments: [...prev.catchments, {
        catchment_id: `C${prev.catchments.length + 1}`,
        area_ha: 1,
        land_use: 'single_family',
        tc_method: 'airport',
      }],
  ...
  const removeCatchment = (index: number) => {
    if (state.catchments.length > 1) {
      setState((prev) => ({
        ...prev,
        catchments: prev.catchments.filter((_, i) => i !== index),
      }));
    }
  };

  const overallStatus = state.result
    ? (state.result.is_capacity_adequate && state.result.is_velocity_adequate ? 'pass' : 'fail')
    : undefined;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
          ...
          <div className="space-y-4">
            {state.catchments.map((catchment, index) => (
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            <InputField
              label="Pipe ID"
              value={state.pipeId}
              onChange={(v) => setState((prev) => ({ ...prev, pipeId: v }))}
              type="text"
              required
            />
            <SelectField
              label="Return Period"
              value={String(state.returnPeriod)}
              onChange={(v) => setState((prev) => ({ ...prev, returnPeriod: parseInt(v) }))}
            ...
            <SelectField
              ...
              onChange={(v) => setState((prev) => ({ ...prev, material: v }))}
            ...
            <InputField
              label="Pipe Slope"
              unit="%"
              value={state.slopePct}
              onChange={(v) => setState((prev) => ({ ...prev, slopePct: v }))}
              placeholder="Auto"
            />
            <InputField
              label="Min Diameter"
              ...
              onChange={(v) => setState((prev) => ({ ...prev, minDiameter: parseInt(v) || 300 }))}
              min={150}
              step={50}
            />
          </div>
        ...
        <button
          ...
          disabled={isPending}
          className="
            w-full py-3 px-4 bg-amber-500 hover:bg-amber-600
      ...
      <div className="space-y-4">
        {state.result ? (
          <>
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  <div className="text-3xl font-mono font-bold text-gray-900">
                    {state.result.diameter_mm} <span className="text-lg text-gray-500">mm</span>
                  </div>
                  <div className="text-xs text-gray-500 uppercase">Pipe Diameter</div>
                </div>
                <CapacityGauge
                  value={state.result.capacity_utilization_percent}
                  label="Capacity"
                  status={state.result.capacity_utilization_percent <= 80 ? 'pass' : 'fail'}
                />
              </div>

              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-gray-500">Design Flow</span>
                  <div className="font-mono text-gray-800">{state.result.design_flow_ls.toFixed(2)} L/s</div>
                </div>
                <div>
                  <span className="text-gray-500">Full Capacity</span>
                  <div className="font-mono text-gray-800">{state.result.full_flow_capacity_ls.toFixed(2)} L/s</div>
                </div>
                <div>
                  <span className="text-gray-500">Velocity</span>
                  <div className="font-mono text-gray-800">{state.result.velocity_ms.toFixed(2)} m/s</div>
                </div>
                <div>
                  <span className="text-gray-500">Slope</span>
                  <div className="font-mono text-gray-800">{state.result.slope_percent.toFixed(3)}%</div>
                </div>
              </div>
            </ResultsCard>
            ...
            <ResultsCard title="Design Checks">
              <div className="flex flex-wrap gap-2">
                <StatusBadge pass={state.result.is_capacity_adequate} label="Capacity" />
                <StatusBadge pass={state.result.is_velocity_adequate} label="Velocity" />
              </div>
            ...
            <ResultsCard title="Pipe Specification">
              <div className="font-mono text-xs space-y-1 text-gray-500">
                <p>Pipe ID: <span className="text-amber-600 font-semibold">{state.result.pipe_id}</span></p>
                <p>Material: <span className="text-gray-700">{state.result.material.toUpperCase()}</span></p>
                <p>Manning's n: <span className="text-gray-700">{state.result.manning_n}</span></p>
                <p>Diameter: <span className="text-gray-700">{state.result.diameter_mm} mm</span></p>
                <p>Slope: <span className="text-gray-700">{state.result.slope_percent.toFixed(4)}%</span></p>
              </div>
            </ResultsCard>
          </>
        ) : (
          <div className="bg-gray-50 border border-gray-200 border-dashed rounded-xl p-8 text-center">
            ...
            <p className="text-gray-500 text-sm">Configure parameters and click Calculate to see results</p>
          </div>
        )}
      </div>
    </div>
...
function SanitaryTab({
  ...
  setProjectName: (name: string) => void;
  state: SanitaryState;
  setState: React.Dispatch<React.SetStateAction<SanitaryState>>;
  presets: PresetScenario[] | undefined;
  presetsLoading: boolean;
  selectedPreset: string;
  onPresetSelect: (key: string) => void;
  ...
  }, [state.result, projectName, state.loads]);
  ...
  const updateLoad = (index: number, field: keyof SanitaryLoadInput, value: string | number) => {
    setState((prev) => {
      const updated = [...prev.loads];
      updated[index] = { ...updated[index], [field]: value };
      return { ...prev, loads: updated };
    });
  };
  ...
  const overallStatus = state.result
    ? (state.result.is_capacity_adequate && state.result.is_velocity_adequate && state.result.is_self_cleansing ? 'pass' : 'fail')
    : undefined;
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <InputField
              ...
              unit="mm"
              value={state.minDiameter}
              onChange={(v) => setState((prev) => ({ ...prev, minDiameter: parseInt(v) || 200 }))}
              min={100}
              step={50}
            />
          </div>
          ...
          <>
            <ResultsCard title="Design Summary" status={overallStatus}>
              <div className="flex items-center justify-between mb-4">
                ...
                <CapacityGauge
                  value={state.result.capacity_utilization_percent}
                  label="Capacity"
                  status={state.result.capacity_utilization_percent <= 75 ? 'pass' : 'fail'}
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  <span className="text-gray-500">Design Velocity</span>
                  <div className="font-mono text-gray-800">{state.result.velocity_ms.toFixed(2)} m/s</div>
                </div>
                <div>
                  <span className="text-gray-500">Min Flow Velocity</span>
                  <div className="font-mono text-gray-800">{state.result.min_flow_velocity_ms.toFixed(2)} m/s</div>
                </div>
              </div>
            </ResultsCard>
            ...
            <ResultsCard title="Design Checks">
              <div className="flex flex-wrap gap-2">
                <StatusBadge pass={state.result.is_capacity_adequate} label="Capacity" />
                <StatusBadge pass={state.result.is_velocity_adequate} label="Velocity" />
                <StatusBadge pass={state.result.is_self_cleansing} label="Self-Cleansing" />
              </div>
              {state.result.notes.length > 0 && (
                <div className="mt-3 space-y-1">
                  {state.result.notes.map((note, i) => (
...
function WaterTab({
  ...
  setProjectName: (name: string) => void;
  state: WaterState;
  setState: React.Dispatch<React.SetStateAction<WaterState>>;
  presets: PresetScenario[] | undefined;
  presetsLoading: boolean;
  selectedPreset: string;
  onPresetSelect: (key: string) => void;
  ...
  const handleExportReport = useCallback(async () => {
    ...
    setIsExporting(true);
    try {
      const totalDwellingUnits = state.loads.reduce((sum, l) => sum + (l.dwelling_units || 0), 0);
      const html = await reportsApi.generateWaterReportHtml({
        project_name: projectName || 'Water Service Calculation',
        building_type: state.loads[0]?.land_use || 'single_family',
        dwelling_units: totalDwellingUnits || 1,
        service_length_m: state.lengthM,
      });
  ...
  }, [state.result, projectName, state.loads, state.lengthM]);
  ...
  const handleCalculate = () => {
    calculate({
      ...
      available_pressure_kpa: state.availablePressure,
      elevation_change_m: state.elevationChange,
      material: state.material,
    });
  };
  ...
  const overallStatus = state.result
    ? (state.result.is_pressure_adequate && state.result.is_velocity_adequate && state.result.is_fire_flow_adequate ? 'pass' : 'fail')
    : undefined;
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            <InputField
              ...
              onChange={(v) => setState((prev) => ({ ...prev, serviceId: v }))}
              type="text"
              required
            />
            <InputField
              ...
              unit="m"
              value={state.lengthM}
              onChange={(v) => setState((prev) => ({ ...prev, lengthM: parseFloat(v) || 0 }))}
              min={1}
              required
            />
            <InputField
              ...
              unit="kPa"
              value={state.availablePressure}
              onChange={(v) => setState((prev) => ({ ...prev, availablePressure: parseFloat(v) || 0 }))}
              min={0}
              required
            />
            <InputField
              ...
              unit="m"
              value={state.elevationChange}
              onChange={(v) => setState((prev) => ({ ...prev, elevationChange: parseFloat(v) || 0 }))}
            />
            <SelectField
              label="Material"
              value={state.material}
      ...
      <div className="space-y-4">
        {state.result ? (
          <>
            <ResultsCard title="Design Summary" status={overallStatus}>
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <div className="text-xs text-gray-500 uppercase">Service Diameter</div>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-mono font-bold text-gray-900">
                    {state.result.residual_pressure_kpa.toFixed(0)}
                  </div>
                  <div className="text-xs text-gray-500 uppercase">kPa Residual</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  <span className="text-gray-500">Head Loss</span>
                  <div className="font-mono text-gray-800">{state.result.head_loss_m.toFixed(2)} m</div>
                </div>
                <div>
                  <span className="text-gray-500">H-W C Value</span>
                  <div className="font-mono text-gray-800">{state.result.hazen_williams_c}</div>
                </div>
              </div>
            </ResultsCard>
            ...
            <ResultsCard title="Design Checks">
              <div className="flex flex-wrap gap-2">
                <StatusBadge pass={state.result.is_pressure_adequate} label="Pressure" />
                <StatusBadge pass={state.result.is_velocity_adequate} label="Velocity" />
                <StatusBadge pass={state.result.is_fire_flow_adequate} label="Fire Flow" />
              </div>
              {state.result.notes.length > 0 && (
                <div className="mt-3 space-y-1">
                  {state.result.notes.map((note, i) => (
...
// ============================================================================
// UNIFIED PRESET SCENARIOS (Maps preset keys to all three calculators)
// ============================================================================

const UNIFIED_PRESETS = [
  { key: 'small_subdivision', name: 'Small Single-Family Subdivision', type: 'residential' },
  { key: 'medium_subdivision', name: 'Medium Single-Family Subdivision', type: 'residential' },
  { key: 'large_subdivision', name: 'Large Single-Family Subdivision', type: 'residential' },
  { key: 'multi_family', name: 'Multi-Family Residential', type: 'residential' },
  { key: 'retail_commercial', name: 'Retail Commercial', type: 'commercial' },
  { key: 'office_commercial', name: 'Office Commercial', type: 'commercial' },
  { key: 'light_industrial', name: 'Light Industrial', type: 'industrial' },
  { key: 'heavy_industrial', name: 'Heavy Industrial', type: 'industrial' },
  { key: 'downtown_infill', name: 'Downtown Infill', type: 'mixed' },
];
...
export function DSSPCalculatorPage() {
  ...
  // Tab-specific states (lifted from child components)
  const [stormwaterState, setStormwaterState] = useState<StormwaterState>(DEFAULT_STORMWATER);
  const [sanitaryState, setSanitaryState] = useState<SanitaryState>(DEFAULT_SANITARY);
  const [waterState, setWaterState] = useState<WaterState>(DEFAULT_WATER);

  // Fetch presets for all tabs
  const { data: stormwaterPresets, isLoading: stormwaterPresetsLoading } = useQuery({
    queryKey: ['stormwater-presets'],
    queryFn: presetsApi.listStormwaterScenarios,
  });

  const { data: sanitaryPresets, isLoading: sanitaryPresetsLoading } = useQuery({
    queryKey: ['sanitary-presets'],
    queryFn: presetsApi.listSanitaryScenarios,
  });

  const { data: waterPresets, isLoading: waterPresetsLoading } = useQuery({
  ...
  // Unified preset loading handler - populates all three tabs
  const handlePresetSelect = useCallback(async (key: string) => {
    ...
    // Try to load preset data for all three tabs
    try {
      // Load stormwater preset
      const stormScenario = await presetsApi.getStormwaterScenario(key).catch(() => null);
      if (stormScenario) {
        setProjectName(stormScenario.name);
        setStormwaterState((prev) => ({
          ...prev,
          returnPeriod: stormScenario.return_period_years,
          catchments: stormScenario.catchments.map((c, i) => ({
      ...
      const sanScenario = await presetsApi.getSanitaryScenario(key).catch(() => null);
      if (sanScenario) {
        setSanitaryState((prev) => ({
          ...prev,
          loads: sanScenario.loads.map((l, i) => ({
      ...
      // Load water preset
      const waterScenario = await presetsApi.getWaterScenario(key).catch(() => null);
      if (waterScenario) {
        setWaterState((prev) => ({
          ...prev,
          lengthM: waterScenario.service_length_m,
          loads: [{
            load_id: 'W1',
            land_use: waterScenario.building_type,
            area_ha: 1,
            dwelling_units: waterScenario.dwelling_units,
            population: Math.round(waterScenario.dwelling_units * waterScenario.occupants_per_unit),
          }],
          result: null, // Clear previous results
        }));
      }
    ...
    <div className="min-h-screen bg-gray-50 relative">
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-8">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-3 py-1.5 mb-4 rounded-lg text-gray-500 hover:text-gray-700 hover:bg-white/80 transition-colors shadow-sm border border-gray-200"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-amber-50 border border-amber-200 rounded-lg shadow-sm">
          ...
          <div className="flex items-center gap-4 mt-4">
            <button
              ...
              className={`
                text-xs px-3 py-1.5 rounded-lg border transition-colors shadow-sm
                ${showIDF
        ...
        {showIDF && (
          <div className="mb-6 bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setShowIDF(false)} className="text-gray-400 hover:text-gray-600">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="min-h-[500px]">
          {activeTab === 'stormwater' && (
            <StormwaterTab
              ...
              state={stormwaterState}
              setState={setStormwaterState}
              presets={stormwaterPresets}
              presetsLoading={stormwaterPresetsLoading}
          ...
          {activeTab === 'sanitary' && (
            <SanitaryTab
              ...
              setProjectName={setProjectName}
              state={sanitaryState}
              setState={setSanitaryState}
              presets={sanitaryPresets}
              presetsLoading={sanitaryPresetsLoading}
              selectedPreset={selectedPreset}
              onPresetSelect={handlePresetSelect}
            />
          )}
          {activeTab === 'water' && (
            <WaterTab
              ...
              presetsLoading={waterPresetsLoading}
              selectedPreset={selectedPreset}
              onPresetSelect={handlePresetSelect}
            />
          )}
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            ...
            <div className="grid grid-cols-3 gap-4 text-xs font-mono">
              <div className="bg-white border border-gray-200 rounded-lg p-3 shadow-sm">
                <div className="text-amber-600 font-semibold mb-1">Stormwater</div>
                <div className="text-gray-700">Min Velocity: {standards.stormwater?.min_velocity || 0.6} m/s</div>
                <div className="text-gray-700">Max Velocity: {standards.stormwater?.max_velocity || 4.5} m/s</div>
                <div className="text-gray-700">Min Cover: {standards.stormwater?.min_cover || 2.0} m</div>
              </div>
              <div className="bg-white border border-gray-200 rounded-lg p-3 shadow-sm">
                <div className="text-amber-600 font-semibold mb-1">Sanitary</div>
                <div className="text-gray-700">Min Velocity: {standards.sanitary?.min_velocity_design || 0.6} m/s</div>
                <div className="text-gray-700">Self-Clean: {standards.sanitary?.min_velocity_self_clean || 0.75} m/s</div>
                <div className="text-gray-700">Infiltration: {standards.sanitary?.infiltration_rate || 0.28} L/s/ha</div>
              </div>
              <div className="bg-white border border-gray-200 rounded-lg p-3 shadow-sm">
                <div className="text-amber-600 font-semibold mb-1">Water</div>
                <div className="text-gray-700">Min Pressure: {standards.water?.min_pressure_normal || 275} kPa</div>
                <div className="text-gray-700">Fire Pressure: {standards.water?.min_pressure_during_fire || 140} kPa</div>
                <div className="text-gray-700">Max Velocity: {standards.water?.max_velocity || 2.5} m/s</div>
              </div>
            </div>
          </div>
        )}
